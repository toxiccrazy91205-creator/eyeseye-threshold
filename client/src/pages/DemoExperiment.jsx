...
function DemoExperiment() {
  ...
  const [conditionName] = useState('Demo Experiment');
  const canvasRef = useRef(null);
  const trialStartTime = useRef(null);
  const timeouts = useRef([]);
  const sessionId = useRef(sessionStorage.getItem('sessionId') || `demo_${Date.now()}`);
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (stimulusVisible && stimulus) {
      ...
      ctx.fillText(stimulus, cx + 100, cy - 50);
    }
    ...
    if (experimentState === 'ready') {
      ...
      ctx.textAlign = 'center';
      ctx.fillText('Demo Experiment - Letter Identification', cx, cy - 60);
  ...
  const runTrial = useCallback(() => {
    ...
    const fixationTimeout = setTimeout(() => {
      ...
      timeouts.current.push(stimulusTimeout);
    }, 1500);
    
    timeouts.current.push(fixationTimeout);
  }, []);
  ...
  const handleResponse = useCallback((key) => {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(correct ? '✓ Correct!' : `✗ It was ${stimulus}`, canvas.width/2, canvas.height/2);
    ...
    timeouts.current.push(feedbackTimeout);
  }, [currentTrial, stimulus, totalTrials, runTrial, conditionName]);

  useEffect(() => {
    const handleKeyDown = (e) => {
      if (responsePhase && e.key.length === 1 && e.key.match(/[a-zA-Z]/)) {
        e.preventDefault();
        ...
        <div className="controls">
          ...
          {experimentState === 'complete' && (
            <div className="results-summary">
              <p>Accuracy: {calculateResults().correct}/{calculateResults().total} ({calculateResults().total > 0 ? Math.round((calculateResults().correct/calculateResults().total)*100) : 0}%)</p>
