...
function TestTiming() {
  ...
  const [conditionName] = useState('Test Timing');
  const canvasRef = useRef(null);
  const trialStartTime = useRef(null);
  const stimulusOnsetTime = useRef(null);
  const timers = useRef([]);
  const sessionId = useRef(sessionStorage.getItem('sessionId') || `timing_${Date.now()}`);
  ...
  const drawCanvas = useCallback(() => {
    ...
    if (experimentState === 'ready') {
      ...
      ctx.textAlign = 'center';
      ctx.fillText('Test Timing - Response Time Measurement', cx, cy - 20);
  ...
  const runTrial = useCallback(() => {
    ...
    const preStimDelay = 1000 + Math.random() * 500;
    ...
    const stimTimeout = setTimeout(() => {
      ...
      requestAnimationFrame(() => {
        ...
        stimulusOnsetTime.current = performance.now();
        trialStartTime.current = Date.now();
        
        const responseTimeout = setTimeout(() => {
          if (experimentState !== 'response') {
            setStimulusVisible(false);
            setExperimentState('response');
            setResponsePhase(true);
          }
        }, 800); // Increased response window slightly for better UX
        
        timers.current.push(responseTimeout);
      });
    }, preStimDelay);
    
    timers.current.push(stimTimeout);
  }, [experimentState]);

  const handleResponse = useCallback((key) => {
    if (!trialStartTime.current) return;
    const reactionTime = Date.now() - trialStartTime.current;
    const webRT = stimulusOnsetTime.current ? performance.now() - stimulusOnsetTime.current : 0;
    const correct = key === stimulus;
    
    setTrialData(prev => ({
      ...prev,
      [currentTrial]: { stimulus, response: key, correct, reactionTime, webRT }
    }));
    
    setTimingInfo({ reactionTime, webRT });
    setResponse({ key, correct, reactionTime, webRT });
    ...
    ctx.textAlign = 'center';
    ctx.fillText(correct ? `RT: ${reactionTime}ms` : `✗ ${stimulus}`, canvas.width/2, canvas.height/2);
    ctx.font = '16px Arial, sans-serif';
    ctx.fillStyle = '#666';
    ctx.fillText(`Web RT: ${Math.round(webRT)}ms`, canvas.width/2, canvas.height/2 + 40);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (responsePhase && e.key.length === 1 && e.key.match(/[XYZ]/i)) {
        e.preventDefault();
  ...
  const calculateResults = useCallback(() => {
    ...
    if (trials.length === 0) return { correct: 0, total: 0, avgRT: 0, avgWebRT: 0 };
    const correct = trials.filter(t => t.correct).length;
    const avgRT = trials.reduce((sum, t) => sum + t.reactionTime, 0) / trials.length;
    const avgWebRT = trials.reduce((sum, t) => sum + (t.webRT || 0), 0) / trials.length;
    return { correct, total: trials.length, avgRT: Math.round(avgRT), avgWebRT: Math.round(avgWebRT) };
  }, [trialData]);
        ...
        <div className="controls">
          ...
          {experimentState === 'complete' && (
            <div className="results-summary">
              ...
              <p>Avg JS RT: {calculateResults().avgRT}ms</p>
              <p>Avg Web RT: {calculateResults().avgWebRT}ms</p>
              <button className="restart-btn" onClick={handleRestart}>Again</button>
            </div>
          )}
        </div>
        {responsePhase && <div className="response-prompt">Press X, Y, or Z</div>}
        {timingInfo.reactionTime && experimentState === 'feedback' && (
          <div className="response-feedback">Reaction Time: {timingInfo.reactionTime}ms | Web Timing: {Math.round(timingInfo.webRT)}ms</div>
        )}
      </div>
    </div>
  );
